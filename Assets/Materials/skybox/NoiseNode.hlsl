...
float gnoise_float(float2 n, out float Out) {
...
float snoise3_float(float3 v, out float Out){ 
...
// lerp final noise value
  ...
  Out = 42.0 * dot( m*m, float4( dot(p0,x0), dot(p1,x1), 
                                dot(p2,x2), dot(p3,x3) ) );
  return Out;
}
//	Simplex 4D Noise 
...
float snoise_float(float4 v, out float Out){
...
// lerp contributions from the five corners
  ...
  m0 = m0 * m0;
  m1 = m1 * m1;
  Out = 49.0 * ( dot(m0*m0, float3( dot( p0, x0 ), dot( p1, x1 ), dot( p2, x2 )))
